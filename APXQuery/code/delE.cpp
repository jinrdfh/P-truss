...
int delE_getGFromVal(MAP_BASIC_G &mpUpG, myG &mpDownG, int iUpK, int iDownK, map <int, MAP_PB_G> &mpAllPbG, long double ldMiniP, long double ldR, myG &mpBaseG)
{
    ...
    if ((ldR < ldMiniP) || (2 >= iDownK))
    {
        mpDownG.addBatch(mpBaseG, ldR);
    }
    else
    {
        debug_assert(mpAllPbG.find(iDownK) != mpAllPbG.end());

        itPbG = mpAllPbG[iDownK].upper_bound(ldR);
        ldDownR = itPbG->first;
        if (itPbG == mpAllPbG[iDownK].begin())
        {
            ldDownR = 0;
        }
        else
        ...
        /* fill down G */
        for (; itPbG != mpAllPbG[iDownK].end(); itPbG++)
        {
            mpDownG.addBatch(itPbG->second, mpBaseG);
        }
    }
    // printf("ldR: %Le upper: %Le down: %Le\n",
    //        ldR, ldUpR, ldDownR);
