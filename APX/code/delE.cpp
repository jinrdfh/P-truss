...
description:
        delete one edge
******************/
int delE_oneE(int x, int y, int iSup, long double ldR, myG &mapG, MAP_BASIC_G &mapUpG, list<int> &lstWaitDel, HLST_DT_E &hlstDtE, MAP_BASIC_G &mapCalG)
{
...
description:
        delete one G
******************/
int delE_subG(int iSup, long double ldUpDt, myG &mapG, MAP_BASIC_G &mapUpG, MAP_PB_G &mapPbG, HLST_DT_E &hlstDtE, MAP_BASIC_G &mapCalG)
{
    ...
    while (!hlstDtE.empty())
    {
        ...
        while (!lstTopBatch.empty())
        {
            ...
            delE_oneE(paXY.first, paXY.second, iSup, ldCurBaseDtE + g_ldAccu, mapG, mapUpG, lstTopBatch, hlstDtE, mapCalG);
        }
    }
...
description:
        delete one G
******************/
int delE_oneG(int iSup, myG &mapG, MAP_PB_G &mapPbG, MAP_PB_G *pmapUpPbG)
{
    MAP_G::iterator itG;
    MAP_BASIC_G::iterator itDegG;
    MAP_BASIC_G mapUpG;
    MAP_BASIC_G mapCalG;
    MAP_BASIC_G mapNewPartG;
    HLST_DT_E hlstDtE;
    MAP_PB_G::iterator itmapPbI;
    ...
    for (itmapPbI = pmapUpPbG->begin(); itmapPbI != pmapUpPbG->end(); itmapPbI++)
    {
        /* get upG */
        delEGetGFromList(mapUpG, itmapPbI->second);
    }

    deTruss_mapSub<MAP_BASIC_G>(mapCalG, mapG.m_mpBasicG, mapUpG);
    mapNewPartG = mapCalG;

    while (!pmapUpPbG->empty())
    {
        itmapPbI = pmapUpPbG->begin();

        calPek_part<MAP_BASIC_G>(mapNewPartG, mapG);

        /* no error */
        debug_assert(delEAddGToDel(iSup, hlstDtE, mapNewPartG, mapG) == false );

        gettimeofday(&tv, NULL);
        lStartTime = tv.tv_sec * 1000 + tv.tv_usec / 1000;
        delE_subG(iSup, itmapPbI->first, mapG, mapUpG, mapPbG, hlstDtE, mapCalG);
        gettimeofday(&tv, NULL);
        ...
        /* next up set */
        delEGetGFromList(mapNewPartG, itmapPbI->second);

        pmapUpPbG->erase(itmapPbI);

        deTruss_mapSub<MAP_BASIC_G>(mapUpG, mapNewPartG);
        deTruss_mapAdd<MAP_BASIC_G>(mapCalG, mapNewPartG);
    ...
    lStartTime = tv.tv_sec * 1000 + tv.tv_usec / 1000;
    /* 1 maximum */
    delE_subG(iSup, 1.1, mapG, mapUpG, mapPbG, hlstDtE, mapCalG);
    gettimeofday(&tv, NULL);
    lCurTime = tv.tv_sec * 1000 + tv.tv_usec / 1000;
    lDelSubTime += lCurTime - lStartTime;
...
description:
        delete part G
******************/
int delE_partG(LIST_DECOMP_G &lstDeG, char *szSaveFile, int iKSpace, myG &oInitG)
{
    ...
    for (itDeG = lstDeG.begin(); itDeG != lstDeG.end(); itDeG++)
    {
        ...
        else
        {
            ...
            mapUpPbG.swap(mapPbG);
            mapPbG.clear();
        }

        gettimeofday(&tv, NULL);
        lStartTime = tv.tv_sec * 1000 + tv.tv_usec / 1000;
        delE_oneG(itDeG->k - 2, mapG, mapPbG, &mapUpPbG);
        gettimeofday(&tv, NULL);
        lCurTime = tv.tv_sec * 1000 + tv.tv_usec / 1000;
